...
static void setSPI_Size(int8_t size){
  ...
  if(config.spi_sz!=size){
    ...
    if(size==mode_16bit){
      ...
      // I don't know if this is at all valid
      #ifdef STM32H7
      // H723 uses lowest 5 bits of CFG1 to hold datasize-1
      LCD_HANDLE.Instance->CFG1 = (LCD_HANDLE.Instance->CFG1 & 0xFFFFFFE0) | SPI_DATASIZE_16BIT;
      #else
      ...
      #ifdef STM32H7
      // H723 uses lowest 5 bits of CFG1 to hold datasize-1
      LCD_HANDLE.Instance->CFG1 = (LCD_HANDLE.Instance->CFG1 & 0xFFFFFFE0) | SPI_DATASIZE_8BIT;
      #else
      LCD_HANDLE.Instance->CR1 &= ~(SPI_CR1_DFF);
      #endif
    }
